...
<script lang="ts">
  ...
  async function enableEncryption() {
    ...
    try {
      if (passphrase) {
        ...
        success = $_('encryption.enabledAutoKey');
      }
  ...
  async function changePassphrase() {
    ...
    try {
      ...
      success = $_('encryption.passphraseChanged');
      showSetup = false;
      passphrase = '';
      confirmPassphrase = '';
    } catch (err) {
      error = err instanceof Error ? err.message : $_('encryption.failedChange');
    } finally {
      loading = false;
    }
  }
...
<div class="encryption-setup">
  <div class="status">
    <div class="status-row">
      ...
      <span class="value" class:enabled={encryptionEnabled}>
        {encryptionEnabled ? `🔒 ${$_('encryption.enabled')}` : `🔓 ${$_('encryption.disabled')}`}
      </span>
    </div>
    {#if encryptionEnabled && keyType}
      <div class="status-row">
        ...
        <span class="value">
          {keyType === 'passphrase' ? `🔑 ${$_('encryption.passphraseBased')}` : `🎲 ${$_('encryption.autoGenerated')}`}
        </span>
      </div>
    {/if}
  </div>
  ...
  {#if !encryptionEnabled}
    ...
    >
      {showSetup ? $_('common.cancel') : $_('encryption.enableButton')}
    </button>
  {:else}
    <div class="actions">
      <button
      ...
      >
        {showSetup ? $_('common.cancel') : keyType === 'passphrase' ? $_('encryption.changePassphrase') : $_('encryption.setPassphrase')}
      </button>
  ...
  {#if showSetup}
    <div class="setup-form">
      <h3>{encryptionEnabled ? $_('encryption.changePassphrase') : $_('encryption.enableButton')}</h3>
      
      <div class="form-group">
        <label for="passphrase">
          {encryptionEnabled ? $_('encryption.newPassphrase') : $_('encryption.passphrase')}
        </label>
        <input
          id="passphrase"
          type="password"
          bind:value={passphrase}
          placeholder={encryptionEnabled ? $_('encryption.enterNewPassphrase') : $_('encryption.leaveEmptyAuto')}
          disabled={loading}
        />
        <small>
          {encryptionEnabled
            ? $_('encryption.changeHint')
            : $_('encryption.leaveEmptyHint')}
        </small>
      </div>
      ...
      >
        {loading ? $_('encryption.processing') : encryptionEnabled ? $_('encryption.changePassphrase') : $_('encryption.enableButton')}
      </button>
    </div>
  {/if}
</div>
